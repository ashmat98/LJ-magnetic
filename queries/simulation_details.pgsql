...
create or replace function roundn(val double precision, n integer)
returns numeric
language plpgsql
as
$$
...
create or replace function tostr(val1 double precision, val2 double precision, n integer)
returns TEXT
language plpgsql
as
$$
...
begin
    ...
    IF val1 = val2 THEN
        return roundn(val1, n);
    ELSE
        return roundn(val1, n) || '-' || roundn(val2, n);
    END IF;
end;
$$;
...
select 
group_name,
tostr(min(eccentricity), max(eccentricity), 2) AS eccentricity,
tostr(min(particles),max(particles),2) as N,
tostr(min(iterations)*AVG(record_interval), max(iterations)*AVG(record_interval),0)  as t,
tostr(min(iterations), max(iterations),0)  as iterations,
count(id),
CASE
